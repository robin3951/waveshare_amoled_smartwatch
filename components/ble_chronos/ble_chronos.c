...
static uint16_t s_pkt_total = 0;   // total bytes expected (3 header + payload length)
...
{
    switch (icon) {
    case 0x01: return "Anruf";
    case 0x02: return "Entgangen";
    case 0x03: return "SMS";
    case 0x09: return "WeChat";
    case 0x0A: return "WhatsApp";
    case 0x0B: return "E-Mail";
    case 0x0C: return "Kalender";
    case 0x0D: return "Google";
    case 0x0E: return "Twitter";
    case 0x0F: return "Facebook";
    case 0x11: return "LinkedIn";
    case 0x12: return "Instagram";
    case 0x13: return "Viber";
    case 0x14: return "Skype";
    case 0x16: return "Line";
    case 0x17: return "Telegram";
    case 0x20: return "Snapchat";
    default:   return "Benachricht.";
    }
}
...
{
    uint8_t pkt[] = {
        0xAB, 0x00, 0x05, 0xFF, 0x91, 0x80,
        (uint8_t)(charging ? 0x01 : 0x00),
        pct
    };
    notify_tx(ch, pkt, sizeof(pkt));
}
...
static void parse(const uint8_t *d, uint16_t len)
{
    ...
    if (c0 == 0x72 && c1 == 0x80 && len >= 9) {
        ...
        ESP_LOGI(TAG, "Notif raw='%.120s' → title='%s' body='%.80s'",
                 raw, title, body);
    ...
    // Device info / version request from app
        ...
        if (conn_handle != BLE_HS_CONN_HANDLE_NONE)
            send_info(conn_handle);
    }
}

/* ── GATT callbacks ──────────────────────────────────────────────────── */

static int rx_cb(uint16_t ch, uint16_t ah,
...
static int tx_cb(uint16_t ch, uint16_t ah,
                 struct ble_gatt_access_ctxt *ctxt, void *arg)
{
...
static const struct ble_gatt_svc_def svcs[] = {
    {
        ...
        .characteristics = (struct ble_gatt_chr_def[]) {
            ...
            {   // TX char — watch notifies phone (no READ; empty read causes ATT error)
                .uuid       = &tx_uuid.u,
                .access_cb  = tx_cb,
...
static void start_adv(void)
{
    // Primary advertisement: flags + name + NUS service UUID (all fits in 31 bytes)
...
static void on_sync(void)
{
    ...
    if (rc) { ESP_LOGE(TAG, "set_rnd rc=%d", rc); return; }
...
void ble_chronos_init(void)
{
    ...
    // Wipe any stale bond/pairing data from NVS so old failed attempts don't block us
    ble_store_clear();
    ...
    if (rc) { ESP_LOGE(TAG, "gatts_count_cfg rc=%d", rc); return; }
    rc = ble_gatts_add_svcs(svcs);
    if (rc) { ESP_LOGE(TAG, "gatts_add_svcs rc=%d", rc); return; }
