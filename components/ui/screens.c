...
static void tileview_changed_cb(lv_event_t *e)
{
    ...
    int idx = 0;
    if      (act == ui_tile_battery) idx = 1;
    else if (act == ui_tile_notif)   idx = 2;
    update_dots(idx);
}
...
static void create_battery_tile(lv_obj_t *tile)
{
    ...
    lv_obj_set_style_text_font(bat_percent_label, &lv_font_montserrat_48, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(bat_status_label, &lv_font_montserrat_16, LV_PART_MAIN);
    lv_obj_set_style_text_color(bat_status_label, lv_color_hex(0x00ff88), LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(bat_voltage_label, &lv_font_montserrat_20, LV_PART_MAIN);
    lv_obj_set_style_text_color(bat_voltage_label, lv_color_hex(0x555555), LV_PART_MAIN);
...
static void create_notif_tile(lv_obj_t *tile)
{
    ...
    lv_obj_set_style_text_align(ble_status_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_set_style_text_font(ble_status_label, &lv_font_montserrat_12, LV_PART_MAIN);
    lv_obj_set_style_text_color(ble_status_label, lv_color_hex(0x555555), LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(notif_empty_label, &lv_font_montserrat_16, LV_PART_MAIN);
    lv_obj_set_style_text_color(notif_empty_label, lv_color_hex(0x444444), LV_PART_MAIN);
    lv_label_set_text(notif_empty_label, "Keine\nBenachrichtigungen");
    lv_obj_set_style_text_align(notif_empty_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
...
static void create_dot_indicator(lv_obj_t *screen)
{
    ...
    lv_obj_clear_flag(dot_container, LV_OBJ_FLAG_SCROLLABLE | LV_OBJ_FLAG_CLICKABLE);
...
void create_screens(void)
{
    ...
    lv_obj_add_event_cb(ui_tileview, tileview_changed_cb, LV_EVENT_VALUE_CHANGED, NULL);
}
...
{
    if (date_label)
        lv_label_set_text_fmt(date_label, "%02d.%02d.%04d", d, mo, y);
}
...
void screens_set_battery(int pct, bool charging, float voltage)
{
    ...
    lv_color_t color;
    if (charging)        color = lv_color_hex(0x00ff88);
    else if (pct < 20)   color = lv_color_hex(0xff3333);
    else if (pct < 50)   color = lv_color_hex(0xffaa00);
    else                 color = lv_color_white();
...
void screens_add_notification(const char *app, const char *msg)
{
    ...
    // If limit reached, delete the oldest bubble (first real child after placeholder)
...
void screens_set_ble_status(bool connected)
{
    ...
    if (connected) {
        ...
        lv_obj_set_style_text_color(ble_status_label, lv_color_hex(0x00ff88), LV_PART_MAIN);
    } else {
        lv_label_set_text(ble_status_label, "BLE: Getrennt");
        lv_obj_set_style_text_color(ble_status_label, lv_color_hex(0x555555), LV_PART_MAIN);
    }
